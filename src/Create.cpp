...
void Player::Create(bool reincarnate)
    ...
    StatiIterNature(this,WEP_SKILL)
        ...
        if ((S->Val >= WS_FOCUSED || wep->Group & WG_EXOTIC) && 
            !wep->HasFlag(IT_NOGEN)) {
...
DoneShield: 
    ...
    for (i=0;SkillKits[i].Val;i++)
      if (SkillRanks[SkillKits[i].Val])
        {
          Item * it;
          it = Item::Create(FIND(SkillKits[i].Text));
          GainItem(it,true);
        }
    ...
    if (getPrimaryMelee() && (InSlot(SL_WEAPON) != getPrimaryMelee()) &&
          !getPrimaryMelee()->isGroup(WG_DAGGERS))
      {
        Item *w, *w2;
        w = getPrimaryMelee();
        w = w->TakeOne();
...
int16 Creature::SkillLevel(int16 sk)
  {
    ...
    if (sk == SK_APPRAISE || sk == SK_INTUITION)
      if (HasFeat(FT_INSIGHTFUL)) /* Depricated */
        s_feat = FEAT_SKILL_BONUS;
