 ...
 */
{
	...
	if (infile != NULL)			/* If #include file	*/
	    infile->line = c_line;		/* Save current line	*/
	infile = file;				/* New current file	*/
	c_line = 1;				/* Note first line	*/
...
newline:
	...
	printf("get(%s), recursion %d, line %d, bptr = %d, buffer \"%s\"\n",
	    file->filename, recursion, c_line,
	...
	if ((c = *file->bptr++ & 0xFF) == EOS) {
		...
		if ((file->bptr = fgets(file->buffer, NBUFF, file->fp))
			...
			printf("\n#line %d (%s), %s",
			    c_line, file->filename, file->buffer);
		    }
#endif
	    ...
	    c_line = infile->line; 		/* Reset line number	*/
	...
	if (instring)				/* Strings just return	*/
        ...
        else if (c == '{') {                    /* Possible Map Grid    */
            instring = TRUE;
            if ((c = get()) != ':')             /* Not a Grid           */
              { unget(); instring = FALSE;      /* Put back the peeked  */
                           return '{'; }        /* char and return '<'  */
            fputc('{',fout); fputc(':',fout);   /* Always keep grids    */
            for (;;) {                          /* Eat a Grid           */
              c = get(); cput(c);
              if (c == EOF_CHAR) {              /* Badly terminated map */
     	    	cerror("EOF in grid", NULLST);
		return (EOF_CHAR);
                }
              else if (c == ':') {              /* Map Terminator      */
                if ((c = get()) != '}')         /* ... or not?         */
                  { cput(c); }                  /* put '}' symbol   */
                else {
                  instring = FALSE;
                  fputc('}',fout);
                  /* Extra return after grid for the sake of
                     the line number that follows */
                  fputc('\n',fout);
	    ...
	    instring = TRUE;			/* So get() won't loop	*/
            line_comment = FALSE;               /* Extended Comment     */
	    if ((c = get()) != '*') {		/* Next byte '*'?	*/
                if (c == '/')                   /* Or a C++ comment?    */
                  {
                    line_comment = TRUE;
		...
		fputc('/',fout);			/* Write out the	*/
		fputc('*',fout);			/*   initializer	*/
	    }
            handle_comment:
	    for (;;) {				/* Eat a comment	*/
		c = get();
 ...
 */
{
	...
	if (*file->bptr == '\n')	/* Ungetting a newline?		*/
	    --c_line;			/* Unget the line number, too	*/
}
 ...
 */
{
	...
	register FILEINFO	*file;
        char buff[120];
	sprintf(buff, "%sline %d, %s: ", MSG_PREFIX, c_line, &severity[1]);
	if (*severity == 'S')
	    sprintf(&(buff[strlen(buff)]), format, arg);
	else
	    sprintf(&(buff[strlen(buff)]), format, (int) arg);
