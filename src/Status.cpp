...
void Thing::CopyStati(Thing *t, int16 n)
  {
    ASSERT(n != TRAP_EVENT);
    StatiIter(t)
      GainTempStati(S->Nature,oThing(S->h),S->Duration,S->Source,
...
void Thing::_FixupStati()
{
  ...
  if (__Stati.Added)
    {
      ...
      if (newSize != __Stati.Allocated)
        {
          Status *newStati;
          newStati = (Status*) x_realloc(__Stati.S,sizeof(Status),
                               newSize,__Stati.Allocated);
          __Stati.S = newStati;
          __Stati.Allocated = newSize;
      ...
      memcpy(__Stati.S+__Stati.Last,__Stati.Added,
                sizeof(Status)*num);
      __Stati.Last += num;
  ...
  for (i=ADJUST;i <= ADJUST_LAST; i++)
    if (__Stati.Idx[i] != 255)
      { __Stati.Idx[ADJUST_IDX] = __Stati.Idx[i]; break; }    
...
void Thing::RemoveStati(int16 n,int8 Cause, int16 Val, int16 Mag,
    Thing *t)
{
      ...
      case CHARMED:
        if (isMonster() && s.h && oThing(s.h)->isPlayer())
          if (s.Val == CH_DOMINATE || s.Val == CH_ALLY || s.Val == CH_COMMAND)
            thism->MakeCompanion(oPlayer(s.h),s.Val == CH_COMMAND ?
                PHD_COMMAND : PHD_MAGIC);
          ...
          if (elapsed)
            {
              ...
              if (ResistLevel(AD_PLYS) == -1 ||
                  ResistLevel(AD_STON) == -1) {
...
void Map::NewField(int32 FType, int16 x, int16 y, uint8 rad,
   Glyph Img, int16 Dur, rID eID, Creature *Creator)
  {
    ...
    for (i=0;f=Fields[i];i++)
      if (dist(x,y,f->cx,f->cy) <= f->rad)
        if ((!FType) || (f->FType & FType))
          if ((!eID) || (f->eID == eID))
            if ((!clev) || (!f->Creator) ||
                  !oThing(f->Creator)->isCreature() ||
                  clev > oCreature(f->Creator)->CasterLev())
              {
                found = true;
                RemoveField(f);
                goto Restart;
...
EvReturn Map::MoveField(Field *f,int16 _cx, int16 _cy, bool is_walk)
  {
    ...
    if (f->FType & FI_SIZE) {
      ...
      for(ix = max(0,_cx - f->rad);ix!=min(sizeX,_cx + f->rad + 1);ix++)
        for(iy = max(0,_cy - f->rad);iy!=min(sizeY,_cy + f->rad + 1);iy++)
          if ((dist(ix,iy,_cx,_cy) <= f->rad) && 
              !(me->canMoveThrough(NULL,ix,iy,blocked_by)))
            {
    ...
    MapIterate(this,t,i) {
      if (!t->isCreature())
        continue;
      /* Avoid brief exit-enter sequence when a player with a field
         around them teleports. */
    ...
    if (ec && f->eID && TEFF(f->eID)->HasFlag(EF_NO_PRESS)) {
      ...
      for (i=0;i!=ec;i++) {
        ...
        if (xe.EActor->isTarget(xe,Entering[i])) {
          if ((!is_walk) || xe.EActor->yn(XPrint(
            "You feel pressure against the <9><Res><5>. Keep going?",xe.eID)))
            {
    ...
    for (f=FFeatureAt(x,y);f;f=NFeatureAt(x,y))
      if (f->isType(T_DOOR))
        if (((Door*)f)->DoorFlags & DF_SECRET)
          return (((Door*)f)->DoorFlags & DF_VERTICAL) ?
            TerrainAt(x,y+1) : TerrainAt(x+1,y);    
        ...
        if (Fields[i]->inArea(x,y))
          {
            ...
            if (!watcher)
              goto noWatcher;
            if (watcher->HasStati(DISB_TERRAIN,-1,
