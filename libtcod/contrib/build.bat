...
@REM if "%BYPASS_VS_CHECK%" NEQ "yes" (
@REM 	REM Ensure that we have a properly set up developer console with access to things like msbuild and devenv.
@REM 	if not exist "%VS140COMNTOOLS%VsDevCmd.bat" (
@REM 		echo You do not appear to have Visual Studio 2015 installed.
@REM 		echo The community edition is free, download it and install it.
@REM 		pause & exit /b
@REM 	)
...
@REM if "%VisualStudioVersion%" NEQ "14.0" (
@REM 	echo Visual Studio incorrect.
@REM 	echo Expected: "14.0"
@REM 	echo Got: "%VisualStudioVersion%"
@REM 	pause & exit /b
@REM )
...
@REM REM Allow the user to specify the path to their Mercurial 'hg' executable.  It may have to be accessed via absolute path.
@REM if not defined HG_EXE (
@REM     hg.exe >nul 2>&1 && (set HG_EXE=hg.exe) || (set HG_EXE=)
@REM )

@REM REM Allow the user to specify the path to their Git 'git' executable.  It may have to be accessed via absolute path.
@REM if not defined GIT_EXE (
@REM     REM This doesn't work directly, like hg.exe, so need where.
@REM     where git.exe >nul 2>&1 && (set GIT_EXE=git.exe) || (set GIT_EXE=)
@REM )

@REM REM If curl is present, which it should be on appveyor, then double check so it can be used for artifact uploads.
@REM if not defined CURL_EXE (
@REM     REM This doesn't work directly, like hg.exe, so need where.
@REM     where curl.exe >nul 2>&1 && (set CURL_EXE=curl.exe) || (set CURL_EXE=)
@REM )

@REM REM Allow the user to specify the path to their 7zip '7z' executable.  It may have to be accessed via absolute path.
@REM if not defined 7Z_EXE (
@REM     7z.exe >nul 2>&1 && (set 7Z_EXE=7z.exe) || (set 7Z_EXE=)
@REM     if "!7Z_EXE!" EQU "" (
@REM         if exist "c:\Program Files\7-Zip\7z.exe" set 7Z_EXE="c:\Program Files\7-Zip\7z.exe"
@REM     )
@REM )
